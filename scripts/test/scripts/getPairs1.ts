...
const bases = GlobalData.bases.BASES_TO_CHECK_TRADES_AGAINST[ChainId.MATIC]
...
function getPairs(tokenA, tokenB) {
...
async function getPath(pairs, tokenA, tokenB, tokenAToSell) {
  // const pairAddresses =
...
const runPath = async () => {
  ...
  const ppp = await getPath(getPairs(MAI, ACRE), MAI, ACRE, new TokenAmount(MAI, '1000000000000000'))
